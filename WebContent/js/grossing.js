...
//    };
    ...
    $scope.printAsset= function (printNp) {
        ...
        $scope.newformatStr = "S2;#1#"+printNp+";#4#NIMHANS  Neuropathology;#5#"+printNp+";";
     	
     	var n=0;
     	
     	//str.includes
//     	if($scope.asset.biopsy == "Nerve"){
//     		n=5;
//     	}else if($scope.asset.biopsy == "Tumour"){
//     		n=1;
//     	}else if($scope.asset.biopsy == "Muscle"){
//     		n=6;
//     	}else{
//     		
//     		n=2;
//     	}
     	...
     	if($scope.asset.biopsy.includes("Epilepsy")){
     	
     		n=6;
     	}else if($scope.asset.biopsy.includes("Muscle")){
     		
     		n=5;
     	}else if($scope.asset.biopsy.includes("Nerve")){
     		
     		n=4;
     	}else if($scope.asset.biopsy.includes("Tumour") || $scope.asset.biopsy.includes("Tumor")){
     		
     		n=1;
     	}else if($scope.asset.biopsy.includes("Lip")){
     		
     		n=3;	
     	}else if($scope.asset.biopsy.includes("Skin")){
     	...
     	var doc ="DT;"+n+";";
     	    doc += "\r\n";
     	    doc += $scope.newformatStr;
     	
         var array = [
                {
                 Np_Number :doc//.replace("|","/").replace("|"," ").replace("|"," ").replace("|"," ")
             }];
     	
         
         alasql('SELECT * INTO TXT("BlockNpNumber.txt") FROM ?',[array]);
  ...
  //alert($scope.queue);
    	//Row No. 1 - DT;n;   
    	//Row No. 2 - S2;#1#1234/20:A:02;#4#NIMHANS  Neuropathology;#5#1234/20:A:02;
    	var text1="DT;n;/n"
    	           "S2;#1#1234/20:A:02;#4#NIMHANS  Neuropathology;#5#1234/20:A:02;"
    	...
    	var jsonObject = JSON.stringify(arr1);
    	//$scope.queue = array_new;
    	//$scope.queue = array_new.concat(arr1);
    	//Array.prototype.push.apply($scope.queue,arr1); 
    	$scope.queue =  $scope.queue.concat(arr1);
       console.log(jsonObject);
    	alasql('SELECT * INTO CSV("BlockNpNumber.txt",{headers:true}) FROM ?',JSON.parse([jsonObject]));
    	//alasql('SELECT * INTO CSV("BlockNpNumber.txt",{headers:true}) FROM ?',[$scope.queue]);
   ...
   //$scope.scanTissue=true;
    ...
    $scope.decalArray = [];
    $scope.obj = {};
    $scope.block_date_array = [];
    $scope.pStatus2 = undefined;
    $scope.date_decalArray =[];
    ...
    $scope.addBlockWithDate = function(){
    	...
    	for(var asset in $scope.assetTasksTable){
    	 ...
    	 //for displaying the selected block with date in the top table
    	 var decaldata_old  = $scope.assetTasksTable[asset].decal+"#Date:"+decalDate;
    	
    	 var decaldata  = decalDate;
    		...
    		if((decalDate == undefined) || (decalDate == "")){
    			...
    			//for saving in db table if there is no decal
    			$scope.decalArray.push(decaldata);
    			//for saving in db if there is decal
    			$scope.block_date_array.push(decaldata_old);
    ...
    //begin code to remove any selected block with date displayed above before saving in db
    $scope.remove_block_date =  function(n){
    	$scope.block_date_array.splice($scope.block_date_array.indexOf(n),1);
    	$scope.decalArray.splice($scope.decalArray.indexOf(n),1);
    	
    }
    //end of code
    
   //either AP or RK or KFF or RK and KFF allowed
    if($("#ap_radio").prop("checked") == true){
    	$("#kff_checkbox").prop("checked",false);
	
	}
    
    //begin code related to In progress for saving the data
    $scope.saveDailyLog = function(){
    	
    	
    
    	//if($('#kff_checkbox').prop("checked",true)){
    	//alert($('#kff_checkbox').val());
    	
    	//begin old code
    	var checkedValue = $('.kf:checked').val();
    	...
    	for(var check in $scope.assetTasksTable){
    		...
    		/*if($('#kff_checkbox_'+check).prop("checked",true)){
    			alert("check")
       		 	$scope.pStatus2 = "KFF";
    		...
    		//if( $scope.pStatus2=="KFF" || $scope.assetTasksTable[check].decal!=undefined){
    		
    		if( $scope.assetTasksTable[check].decal!=undefined){
    			
    			
    			var kffDate = $('#'+check+'_kffDate').val();
    			var decalDate = $('#'+check+'_decalDate').val();

//    			if(kffDate == '' &&  $scope.pStatus2=="KFF"){
//    				
...
//    			}
    			...
    			if($scope.decalArray == '' && ($scope.assetTasksTable[check].decal!=undefined && $scope.assetTasksTable[check].decal[0]!='undefined')){
    	...
    	//table is asset table in db and column is process_status for saving the in progress data
    	for(var asset in $scope.assetTasksTable){
    		
        
    		/*if($('#'+asset+'kff_checkbox').prop("checked",true)){
    			alert($('#'+asset+'kff_checkbox').val());
    		...
    		//if(($scope.assetTasksTable[asset].pStatus != undefined && $scope.assetTasksTable[asset].decal != undefined) || ($scope.pStatus2!=undefined && $scope.assetTasksTable[asset].decal!=undefined)){
    		if(($scope.assetTasksTable[asset].pStatus != undefined ) || ($scope.assetTasksTable[asset].pStatus2 !="" )){	
    			
        		
    			//for RK and KFF data
    			//if($scope.assetTasksTable[asset].pStatus!=undefined && ($scope.assetTasksTable[asset].pStatus == "KFF" || $scope.assetTasksTable[asset].pStatus == "RK")){
    			//	if($scope.decalArray.length < 1){
    				
    				
    				var kffDate = $('#'+asset+'_kffDate').val();
    				
    				
       				if($scope.assetTasksTable[asset].pStatus=="RK" && $scope.assetTasksTable[asset].pStatus2 !="KFF")
    					$scope.pStatus = $scope.assetTasksTable[asset].pStatus+":"+date;
    				else if($scope.assetTasksTable[asset].pStatus2=="KFF" && $scope.assetTasksTable[asset].pStatus==undefined)
    					$scope.pStatus = $scope.pStatus2+":"+kffDate;
    				else if($scope.assetTasksTable[asset].pStatus=="RK" && $scope.assetTasksTable[asset].pStatus2 =="KFF" )
    					$scope.pStatus =$scope.assetTasksTable[asset].pStatus+":"+date+","+ $scope.pStatus2+":"+kffDate;  				
    				else if($scope.assetTasksTable[asset].pStatus=="AP")
    				...
    				//for(var j1 in $scope.assetTasksTable[0].blocks){
    					
       				//REST call for updating the process_status of tissue NP number in asset table
    				$http.post(ASSET_URL_BASE+"/tissue/setProcessStatus?npNumber="+$scope.assetTasksTable[asset].npNumber+"&pStatus="+$scope.pStatus)
    			        .success(function(data) {
    			        	
    			        	////REST call for updating the process_status of block NP numbers in asset table with the Decal date
    			        	
    			        	for(var j1 in $scope.assetTasksTable[0].blocks){
    			        		
    			        		//if there is no decal for a tissue
    			        		if($scope.decalArray[j1] == undefined){
    			        			$scope.decalArray[j1] = "";
    			        			
    			        		}
    			        		
    			        		//if there is decal data for a tissue
    			        		if($scope.block_date_array.length > 0){
    			        			
    			        		for(var j2 = 0;j2<$scope.block_date_array.length;j2++){
        			        		$scope.date_decalArray = $scope.block_date_array[j2].split("#Date:");
        			        		
        			        		
    			        		 $http({
    			                    	url : ASSET_URL_BASE+"/block/setDecal?npNumber="+$scope.date_decalArray[0]+"&pStatus=Decal:"+$scope.date_decalArray[1],
    			                        method : "POST"
    			        			...
    			        			//if there is no decal data for tissue
    			        			$http({
    			                    	url : ASSET_URL_BASE+"/block/setDecal?npNumber="+$scope.assetTasksTable[0].blocks[j1].npNumber+"&pStatus = Decal:"+$scope.decalArray[j1],
    			                        method : "POST"
    			                    }).success(function(res){
    			...
    			//if($scope.assetTasksTable[asset].decal!=undefined && ($scope.assetTasksTable[asset].pStatus !="KFF" || $scope.assetTasksTable[asset].pStatus !="RK")){
    				//if($scope.assetTasksTable[asset].pStatus == "AP" && ($scope.assetTasksTable[asset].pStatus !="KFF" || $scope.assetTasksTable[asset].pStatus !="RK")){
    				//if($scope.decalArray.length > 0){
    				...
    				/*for (var j in $scope.assetTasksTable[asset].decal){
    					console.log($scope.assetTasksTable[0].blocks[j].npNumber);
    					for(var j1 in $scope.assetTasksTable[0].blocks){
    						alert($scope.assetTasksTable[0].blocks[j1].npNumber+" -- "+$scope.decalArray[j1]);
    					}

    				for(var j1 in $scope.assetTasksTable[0].blocks){
    					alert($scope.decalArray[j1]);
                    $http({
                    	url : ASSET_URL_BASE+"/block/setDecal?npNumber="+$scope.assetTasksTable[0].blocks[j1].npNumber+"&pStatus = Decal:"+$scope.decalArray[j1],
                        method : "POST"
                    }).success(function(res){
    ...
    //begin code for date field and calender upon clicking KFF
    $scope.enableDatePicker = function(pStatus2){
    	
    
    	if(pStatus2 == "KFF"){
    	$("#ap_radio").attr("checked",false);
...
$scope.getNCS = function(np_new){
	...
	sessionStorage.setItem("station", "2");
